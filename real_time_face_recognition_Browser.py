...
def stream(pb_path, node_dict,ref_dir,camera_source=0,resolution="480",to_write=False,save_dir=None):
    ...
    while(cap.isOpened()):
        ...
        if ret is True:
            ...
            #----image display
            # cv2.imshow("demo by Vinayak", img)
            ret1, img1 = cv2.imencode('.jpg', img)
            frame = img1.tobytes()
            yield (b'--frame\r\n'
...
def video_feed():
    return Response(stream(pb_path, node_dict, ref_dir, camera_source=camera_source, resolution="720", to_write=False, save_dir=None), mimetype='multipart/x-mixed-replace; boundary=frame')

if __name__ == "__main__":
    camera_source = 0 #can be also the path of a clip
    # pb_path = r"G:\我的雲端硬碟\xxx\pb_model_select_num=15.pb"
    pb_path = r"C:/Users/Dell/PycharmProjects/JohnnyAI/Section16_Training/pb_model_select_num=2.pb"
    ...
    ref_dir = r"C:/Users/Dell/PycharmProjects/JohnnyAI/Section17_Realtime_FaceRec_Desktop/Realtime_test"
    app.run(debug=True)
